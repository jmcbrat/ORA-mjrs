CREATE OR REPLACE
package body      MPK_ppmjrs_Batch_Processing is
...
--------------------------------------------
	...
	*/  
	...
	BEGIN
		...
		INSERT 
		  INTO pp_OFFTRK_IMPORT_BATCHES
	           (batch_id, IMPORT_DATE, TRANSACTIONS, charges,inmate,trans_log_start,trans_log_end)
	    VALUES
	           (pp_import_batches.nextval,
	            sysdate,
	            0,
	            0,
	            0,
	            pp_TRANSACTIONLOG_SEQ.NEXTVAL,
	            null 
	           );
	
		select pp_import_batches.currval into v_batch_id from dual;
	...
	*/ 
	...
	BEGIN   
		UPDATE OFFTRK_IMPORT_BATCHES o
	       SET (TRANSACTIONS) = (select (transactions+1) from pp_OFFTRK_IMPORT_BATCHES o2 where o.batch_id = o2.batch_id)
	...
	*/ 
	...
	BEGIN   
		UPDATE OFFTRK_IMPORT_BATCHES o
	       SET (INMATE) = (select (INMATE+1) from pp_OFFTRK_IMPORT_BATCHES o2 where o.batch_id = o2.batch_id)
	    where batch_id = p_batch_id;
	    commit;
	...
	*/ 
	...
	BEGIN   
		UPDATE OFFTRK_IMPORT_BATCHES o
	       SET (CHARGES) = (select (CHARGES+1) from pp_OFFTRK_IMPORT_BATCHES o2 where o.batch_id = o2.batch_id)
	    where batch_id = p_batch_id;
	    commit;
...
--------------------------------------------------
	...
	*/  
	...
	begin
		...
		BEGIN
	    	insert into pp_OFFTRK_IMPORT_INMATE_LOG (batch_id, OFFENDER_ID, NAME_FIRST, NAME_FAMILY, NAME_OTHER, STREET, CITY_TOWN_LOCALE,
	...
	*/  
	...
	BEGIN
		...
		END IF;
		INSERT INTO pp_OFFTRK_IMPORT_CHARGE_LOG 
		SELECT p_batch_id, OFFENDER_ID, CHARGE_ID, pp_TRANSACTIONLOG_SEQ.currval, sysdate, TRANSACTION_CODE, 
	...
	*/  
	...
	begin
	   ...
	   IF (p_Status = 'TRUE') THEN
			...
			BEGIN
				INSERT INTO pp_INMATES_STMT_INFO
					(OFFENDER_ID, 
	...
	*/  
	...
	begin
	    ...
	    -- inmates
	    begin 
	    	insert into pp_inmates (OFFENDER_ID, NAME_FIRST, NAME_FAMILY, NAME_OTHER, STREET, CITY_TOWN_LOCALE,
			STATE_PROVINCE_REGION, POSTAL_CODE, PHONE_NUMBER, ALTERNATIVE_PHONE_NUMBER,  
	 	...
	 	IF p_charge_detail_id > 0 THEN  -- must already have a record to end date it.
	 		...
	 		BEGIN
		 		SELECT distinct STATUS_START_DATE
		 		  INTO v_start_date 
				  FROM pp_charge_detail --epic.eh_booking_custody_status bcs
				 WHERE booking_id = p_booking_id
	...
	*/  
	...
	BEGIN
		...
		INSERT INTO pp_charges
           ...
           UPDATE pp_charge_detail            -- add the charge_id to charge_detail record....
			...
			UNION  -- create a new inmate
				SELECT distinct id.offender_id AS OFFENDER_ID,
					...
					WHERE  b.booking_id = c.booking_id
					  ...
					  AND  c.sentence_id = s.sentence_id
					  AND  b.entity_id = id.entity_id
					  AND  epic.ef_epic_date_to_date(date_entered) BETWEEN v_last_run_date
					  											       AND sysdate
	       ...
	       -- if was (this booking) on work release skip DO NOT TOUCH per Lori
			...
			if gv_debug = TRUE and gv_debug_inmate = records.offender_id THEN
				...
				IF (records.STATUS = 'NEW SENTENCE' or records.STATUS = 'NEW MISSING' or v_inmate_exists = 'NO') THEN 					    
					...
					BEGIN
						SELECT 'YES' INTO v_inmate_exists FROM pp_inmates i WHERE i.offender_id = records.offender_id;  
					EXCEPTION WHEN no_data_found THEN
			...
			END IF;		
 		END LOOP;
		update pp_OFFTRK_IMPORT_BATCHES
		set trans_log_end = pp_TRANSACTIONLOG_SEQ.CURRVAL
		where batch_id = v_batch_id;
		commit;
 	END;
 	
END MPK_ppmjrs_Batch_Processing;
/
