CREATE OR REPLACE
PROCEDURE MP_mjrs_inmate_update (
...
begin
    ...
    begin 
    	update inmates 
    	set (   NAME_FIRST, NAME_FAMILY, NAME_OTHER, STREET, CITY_TOWN_LOCALE,
                ...
                DATE_OF_BIRTH) =
				 (select pi.NAME_FIRST as NAME_FIRST,
						 ...
						 decode(p_custody,' ', (select account_status_id from ACCOUNT_STATUS_TYPES where account_status_desc = 'Inactive'),
						                       (select account_status_id from ACCOUNT_STATUS_TYPES where account_status_desc = 'Active')),
		...
		WHERE   inmates.offender_id = p_offender_id;
	EXCEPTION 
